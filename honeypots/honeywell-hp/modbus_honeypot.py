...
def handle_read_coils(tid, uid, start, qty):
    with LOCK:
        # Toggle each coil state
        for i in range(start, start + qty):
            key = str(i)
            COILS[key] = 0 if COILS.get(key, 0) else 1

        # Reconstruct bits after toggle
        bits = ''.join(str(COILS.get(str(i), 0)) for i in range(start, start + qty))
        padded = bits.ljust(((len(bits)+7)//8)*8, '0')
        byte_val = int(padded[::-1], 2).to_bytes(len(padded)//8, 'little')
        payload = b'\x01' + bytes([len(byte_val)]) + byte_val
